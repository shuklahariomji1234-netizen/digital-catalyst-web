...
const Analytics: React.FC<AnalyticsProps> = ({ orders, products, users, reviews }) => {
    ...
    completedOrders.forEach(order => {
        order.items.forEach(item => {
            ...
            const existing = productSales.get(item.id as number);
            if (existing) {
                existing.quantity += item.quantity;
            } else {
                productSales.set(item.id as number, { name: productInfo.title, quantity: item.quantity });
            }
        });
    });
                        ...
                        <div className="space-y-2">
                            {visibleWishlist.map((product) => (
                                <VisualBar 
                                    key={product.id}
                                    label={product.title}
                                    value={Number(product.wishlistCount || 0)}
                                    max={Number(maxWishlistCount)}
                        ...
                        <div className="space-y-2">
                            {visibleReviews.map((product) => (
                                <VisualBar 
                                    key={product.id}
                                    label={product.title}
                                    value={Number(product.actualReviewCount)}
                                    max={Number(maxReviewCount)}
